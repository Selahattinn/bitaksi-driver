...
import (
	...
	"github.com/Selahattinn/bitaksi-driver/internal/api"
...
func NewInstance(cfg *Config) (*Instance, error) {
	...
	repository, err := repository.NewMongoRepository(cfg.DB)
	if err != nil {
		logrus.WithError(err).Fatal("Failed to start MongoDB")
	}

	// Initialize ServiceProvider
	serviceProvider, err := service.NewProvider(cfg.Service, repository)
	if err != nil {
